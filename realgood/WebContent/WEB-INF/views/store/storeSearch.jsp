		 ...
		 <div style = "clear:both">
              <ul class="pagination">
                 <% if(currentPage>10){ %>
                    <!-- 맨 처음 페이지로 이동[<<] -->
                    <li>
					 <a class="page-link" href="<%=request.getContextPath() %>/store/search.do?group=<%=group %>&address=<%=address%>&cp=1">&lt;&lt;</a>
                    </li>
                    
                    <!-- 이전 순번의 페이징 바로 이동[<] -->
                    <li>
					 <a class="page-link" href="<%=request.getContextPath() %>/store/search.do?group=<%=group %>&address=<%=address%>&cp=<%=prev%>">&lt;</a> 
                    </li>
                    <% } %>
                       <!-- 10개의 페이지 목록 -->
                       <% for(int p = startPage; p<=endPage; p++){%>
                          ...
                          <% }else{%>
                                <li><a class="page-link" href="<%=request.getContextPath()%>/store/search.do?group=<%=group %>&address=<%=address%>&cp=<%=p%>"><%=p%></a></li>
                          <% } %>
                       <% } %> 
                       <% if(next<maxPage){ %>
                          <!-- 다음 페이징바 [>] -->
                          <li><a class="page-link" href="<%=request.getContextPath()%>/store/search.do?group=<%=group %>&address=<%=address%>&cp=<%=next%>">&gt;</a></li>
                       
                          <!-- 마지막 페이지로 이동 [>>] -->
                          <li><a class="page-link" href="<%=request.getContextPath() %>/store/search.do?group=<%=group %>&address=<%=address%>&cp=<%=maxPage%>">&gt;&gt;</a></li>
                 <% } %>		 
