...
<body>
    ...
    <div class="row my-5">
     ...
     <%--        <form action="<%=request.getContextPath()%>/member/memberManage.do" method="POST"> --%>
                ...
                <button id="deleteBtn" class="btn btn-primary">삭제하기</button>
                                   전체<input id="checkAll" class="btn btn-primary" type="checkbox" name="check" >
            <!-- </form> -->
    ...
    $("#deleteBtn").on("click", function() {
        ...
        $.each(checkbox, function(index, item) {
           ...
           id = td.eq(2).text();
           gradeName = td.eq(7).text();
           // 가져온 값을 배열에 담는다.
           //tdArr.push(id);

          // console.log(id);
           gradeNameSum += gradeName + " ";
           idSum += id + " ";
        });
        console.log(1);
        if(confirm("정말로 삭제하시겠습니까?")){
        	console.log(idSum);
        	console.log(gradeNameSum);
        	location.href = "<%=request.getContextPath()%>/member/memberDelete.do?id=" + idSum + "&gradeName=" +gradeNameSum;
        }
     });
