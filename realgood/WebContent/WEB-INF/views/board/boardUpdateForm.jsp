...
<html >
	...
	<%@ include file="../common/header.jsp"%>
      ...
      <section class="ReviewWritingPage__Container">
        ...
        <div class="ReviewWritingPage__Row">
          <h5 class="RestaurantSubMessage__RestaurantName"><strong><%=board.getNickName() %> </strong></h5>
          <div class="RestaurantSubMessage__SubMessageWrap">
            <span class="RestaurantSubMessage__SubMessage">에 대한 솔직한 리뷰를 수정해주세요.</span>
          </div>
        </div>
 ...
 <script>
		...
		$("#category > option").each(function(index, item){
			// 옵션 6개가 배열의 형태 / each(jquery 반복문) index 반복하는 순서 0~5까지 반복 / item : 현재 접근하고 있는 요소
			if($(item).val() == "<%=board.getBoardNo()%>"){
				// 현재 접근한 요소의 value를 꺼내서 board.getCategoryName과 같으면
				$(item).prop("selected","true");
				// 같은 옵션 태그에 selected를 부여함 selected는 true로 부여해야 함
			}
		});
